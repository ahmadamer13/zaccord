...
let countries = ["Albania", "Andorra", "Argentina", "Australia", "Austria", "Azerbaijan",
  ...
  "Ireland", "Israel", "Italy", "Japan", "Kazakhstan", "South Korea",
  "Kuwait", "Jordan",
  "Latvia", "Liechtenstein", "Lithuania", "Luxembourg", "Macedonia", "Malaysia", "Malta",
  ...
  "Uruguay", "Uzbekistan", "Vatican City", "Venezuela", "Vietnam", "Serbia",
  "Kosovo",
  "Montenegro"];
...
_('diffBilling').addEventListener('click', function toggleForm(e) {
  if (this.getAttribute('data-status') != 'close') {
    ...
    for (let i = 0; i < countries.length; i++) {
      let selected = '';
      if (countries[i] === 'Jordan' || countries[i] === 'Hungary') selected = 'selected';
...
function packetaSelectPoint(point) {
  if (point && point.error !== null) {
    ...
    _('selectedPacketaPoint').innerHTML = 'This pickup point is currently unavailable';
  } else if (point !== null) {
    _('selectedPacketaPoint').style.display = 'block';
